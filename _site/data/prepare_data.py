...
def get_repos_from_github(user):
    ...
    for page in range(1, int(public_repos_count / PAGE_SIZE) + 2):
        ...
        logging.debug(repo_url % (PAGE_SIZE, page))
...
@click.option('--existing-file', type=str, required=False, default='projects.json', help='The existing data file to merge with (defaults to projects.json).')
@click.option('--output-file', type=str, required=True, default='updated_projects.json', help='The data output file (defaults to projects_new.json).')
